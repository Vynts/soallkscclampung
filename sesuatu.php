...
include("config/config.php");
 if($_GET['data']=="sesuatuhome") {?>
    <div class="row">
        <?php
            $hasil = mysqli_query($conn, "SELECT * FROM `t_sesuatu` ORDER BY merk_sesuatu ASC");
            while ($data = mysqli_fetch_array($hasil)) {
            
                echo '<div class="col-sm-6 col-md-4">';
                    echo '<div class="thumbnail">';
                        echo '<img src="images/sesuatu/'.$data['gambar'].'" width=70%>';
                        echo '<div class="caption">';
                            echo '<h3>'.$data['merk_sesuatu'].' '.$data['nama_sesuatu'].'</h3>';
                            echo '<h5>Harga Sewa : Rp. ' . number_format($data['harga_sewa']) . ' ,- / Hari</h5>';
                            echo '<p align=center>';
                            if($data['status_sesuatu']=='0'){
...
<?php } elseif($_GET['data']=="sesuatulacak") { ?>
    <div class="row">
        ...
        <script>
            ...
            <?php 
                while ($data = mysqli_fetch_assoc($run)) : 
                    $sqll = mysqli_query($conn, "SELECT * FROM t_sesuatu where id_sesuatu = '$data[id_sesuatu]'");
                    $data1 = mysqli_fetch_assoc($sqll);
                    $nosesuatu = $data1['no_sesuatu'];
                    $merk = $data1['merk_sesuatu'];
                    $namasesuatu = $data1['nama_sesuatu'];
            ?>
            L.marker([<?php echo $data['x']; ?>, <?php echo $data['y']; ?>],{icon: bluemarker}).addTo(mymap)
                .bindPopup("<b>Mobil</b> : <br><?php echo $nosesuatu; ?><br><?php echo $merk; ?> <?php echo $namasesuatu; ?>
